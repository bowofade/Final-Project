...
from .utils import request_bfade
...
class Store(object):
    ...
    def get_details(self):
        details = request_bfade(self.urls.info_url(), store_id=self.id)
        return details
    ...
    def get_menu(self, lang='en'):
        response = request_bfade(self.urls.menu_url(), store_id=self.id, lang=lang)
...
class StoreLocator(object):
    ...
    def nearby_stores(address, service='Delivery'):
        ...
        """
        data = request_bfade(address.urls.find_url(), line1=address.line1, line2=address.line2, type=service)
