...
class Customer:
    ...
    def save(self, filename="customers/customer1.pizzapy"):
        """
        saves the current customer to a .pizzapy file for loading later
        """
        if not filename.startswith("customers"):
            filename = "customers/" + filename
        pizzapy_dict = {"first_name": self.first_name,
        ...
        with open(filename, "w") as f:
            pizzapy.dump(pizzapy_dict, f)

    @staticmethod
    def load(filename):
        """
        load and return a new customer object from a pizzapy file
        """
