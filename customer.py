...
class Customer:
    ...
    def save(self, filename="customers/customer1.bfade"):
        """
        saves the current customer to a .bfade file for loading later
        """
        if not filename.startswith("customers"):
            filename = "customers/" + filename
        bfade_dict = {"first_name": self.first_name,
        ...
        with open(filename, "w") as f:
            bfade.dump(bfade_dict, f)

    @staticmethod
    def load(filename):
        """
        load and return a new customer object from a bfade file
        """
