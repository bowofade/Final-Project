class Store(object):
    ...
    """
    def __init__(self, data={}, country=COUNTRY_):
    ...
    def get_details(self):
        details = request_pizzapy(self.urls.info_url(), store_id=self.id)
        return details
    ...
    def get_menu(self, lang='en'):
        response = request_pizzapy(self.urls.menu_url(), store_id=self.id, lang=lang)
...
class StoreLocator(object):
    ...
    def nearby_stores(address, service='Delivery'):
        """Query the bfade to find nearby stores.

        nearby_stores will filter the information we receive from the bfade
        to exclude stores that are not currently online (!['IsOnlineNow']),
        and stores that are not currently in service (!['ServiceIsOpen']).
        """
        data = request_python(address.urls.find_url(), line1=address.line1, line2=address.line2, type=service)
